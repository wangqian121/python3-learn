...
try:
    ...
    print('please install talib')

def EMA(DF, N):
    return pd.Series.ewm(DF, span=N, min_periods=N - 1, adjust=True).mean()


def SAR(DF, acceleration=0, maximum=0):
    res = talib.SAR(DF.high.values, DF.low.values, acceleration, maximum)
    return pd.DataFrame({'SAR': res}, index=DF.index)
